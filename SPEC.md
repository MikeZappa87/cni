...
- `requiredCapabilities` (list of strings): Linux capabilities (e.g. `["NET_ADMIN", "SYS_ADMIN"]`) that this plugin requires. If omitted, the plugin is assumed to require root. When specified, the runtime SHOULD drop all capabilities not in this list before executing the plugin. Plugins that need to perform privileged operations not covered by their declared capabilities may instead use the runtime gRPC socket (see `runtimeSocket` below).
- `runtimeSocket` (boolean): If `true`, the plugin requires a gRPC socket connection back to the container runtime for delegating privileged operations. The runtime MUST provide the socket path via the `CNI_RUNTIME_SOCKET` environment variable when this is set.
...
- `CNI_RUNTIME_SOCKET` (optional): An absolute path to a Unix domain socket provided by the container runtime. Plugins that declare `"runtimeSocket": true` in their configuration may use this socket to make gRPC calls back to the runtime for privileged operations (e.g., operations normally requiring `NET_ADMIN` or `SYS_ADMIN`).
...
Traditionally, CNI plugins execute with full root privileges. To support the principle of least privilege, this specification introduces two complementary mechanisms:

#### Required Linux Capabilities

A plugin may declare the exact set of Linux capabilities it needs via the `requiredCapabilities` key in its plugin configuration object. For example:
...
When `requiredCapabilities` is present, a runtime SHOULD drop all Linux capabilities not in this list before executing the plugin binary. If `requiredCapabilities` is absent or empty, the runtime MUST assume the plugin requires full root privileges (preserving backwards compatibility).

The valid values for `requiredCapabilities` are Linux capability names without the `CAP_` prefix (e.g. `"NET_ADMIN"`, `"SYS_ADMIN"`, `"NET_RAW"`, `"DAC_OVERRIDE"`).

#### Runtime Socket for Delegated Privileged Operations

Some plugins can be redesigned to require no elevated capabilities at all, instead delegating privileged operations (e.g. creating veth pairs, setting up iptables rules, configuring network namespaces) to the container runtime via a gRPC socket.

A plugin that supports this model declares `"runtimeSocket": true` in its configuration:
...
When a plugin declares `runtimeSocket`, the runtime MUST provide the `CNI_RUNTIME_SOCKET` environment variable containing the absolute path to a Unix domain socket. The plugin can then issue gRPC requests to the runtime to perform privileged operations on its behalf.

The gRPC service that a runtime exposes on the socket is defined by the container runtime. For example, containerd exposes a `PodNetworkLifecycle` service for core lifecycle operations and an optional `PodNetworkManagement` service for lower-level primitives:

```protobuf
// Core lifecycle — required for all networking plugins.
...
// Extended management — optional, for advanced use cases
// (DRA, multi-NIC, SR-IOV, RDMA device management).
...
See the [containerd networking proto](https://github.com/MikeZappa87/containerd/blob/9e3dd9176bb51b1b19b24e20e870262d928594f5/api/services/networking/v1/networking.proto) for the full message definitions. The `PodNetworkManagement` service provides fine-grained privileged operations (creating veth pairs, assigning IPs, applying routes, moving devices) that allow plugins to run without `NET_ADMIN` or `SYS_ADMIN` capabilities by delegating those operations to the runtime.
